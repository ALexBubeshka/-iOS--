...
class ScrollGalleryController: UIView {
   ...
   private func loadFromNib() -> UIView {
       ...
       let nib = UINib(nibName: "ScrollGalleryController", bundle: bundle)
       guard let view = nib.instantiate(withOwner: self, options: nil).first as? UIView else {return UIView()}
       return view
   }
   func getIndexPath(imageGallerySelectedIndexPath:Int){
       self.currentIndex = imageGallerySelectedIndexPath
   }
   ...
   func setImages(images: [UIImage], imageGallerySelectedIndexPath: Int) {
       getIndexPath(imageGallerySelectedIndexPath: imageGallerySelectedIndexPath)
       self.images = images
       if self.images.count > 0 {
           self.mainImageView.image = self.images[imageGallerySelectedIndexPath]
       }
